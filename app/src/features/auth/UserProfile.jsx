  ...
  return (
    <div className="flex items-center gap-3 bg-indigo-800/50 p-1.5 pr-4 rounded-full border border-indigo-500/30 backdrop-blur-sm">
      ...
      <div className="hidden md:block">
        <p className="text-xs font-bold text-white leading-tight max-w-[100px] truncate">{user.displayName?.split(' ')[0]}</p>
        {isAdmin && (
      ...
      <button onClick={handleLogout} className="ml-2 text-indigo-200 hover:text-white transition-colors" title="Sair">
        <LogOut size={16} />
      </button>
    </div>
  );
