            ...
            <div key={poll.id} className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 p-4">
              ...
              <div className="space-y-3 mb-4">
                {options.map((option) => {
                  ...
                  return (
                    <div key={option.id} className={`relative ${!showResults && !isClosed ? 'cursor-pointer group' : ''}`} onClick={() => !showResults && handleVote(poll, option.id)}>
                      <div className="flex justify-between text-[11px] mb-1 px-1">
                        <span className="text-slate-700 dark:text-slate-300">{option.text}</span>
                        {showResults && <span className="text-slate-400">{percentage}%</span>}
                      </div>
                      <div className="h-2 bg-slate-100 dark:bg-slate-700 rounded-full overflow-hidden">
                        {showResults && <div className="h-full bg-emerald-600 rounded-full transition-all duration-500" style={{ width: `${percentage}%` }} />}
                      </div>
                    </div>
                  );
                })}
              ...
              {showResults && <p className="text-[11px] text-slate-400 text-right mb-2">{totalVotes} voto{totalVotes !== 1 ? 's' : ''}</p>}
              ...
              {hasVoted && !isClosed && (
                <p className="text-center text-[11px] text-emerald-600 mt-2 flex items-center justify-center gap-1">
